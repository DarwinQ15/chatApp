...
const getConversationMessages = async (req, res, next) => {
  ...
  const createMessageInConversation = async (req, res, next) => {
    try {
      const data = req.body; // sacamos el senderId, message
      const {conversationId} = req.params;
      const result = await ConversationsServices.createMessage({...data, conversationId});// esto es lo mismo de lo que tengo arriba
      res.json(result);
    } catch (error) {
      next({
        status: 400,
    ...
    try {
      ...
      const messages = await ConversationsServices.getMessages(
        ...
        res.json({
          ...
          previus: `${process.env.HOST}/api/v1${req.path}?offset=${offset - limit}&limit=${limit}`,
          messages: rows,
        });
    } catch (error) {
      next({
